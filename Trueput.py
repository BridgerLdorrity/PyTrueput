import win32, win32con,win32api,win32gui
import time
import ctypes
...
def move(x,y,timetomoveseconds=0,timeinterval=100):
    if timetomoveseconds != 0:
        x = x//timetomoveseconds//timeinterval
        y = y//timetomoveseconds//timeinterval
        for i in range(timetomoveseconds * timeinterval):
...
def moveto(x,y,timetomoveseconds=0,timeinterval=100):
    if timetomoveseconds != 0:
        curpos = win32api.GetCursorPos()
        curpos = list(curpos)
        y = y - curpos[0]
    ...
    if timetomoveseconds == 0:
        ...
        win32api.SetCursorPos(curpos)
def leftclick(buffer = .1,amount = 1):
    for i in range(amount):
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTDOWN,0,0)
        time.sleep(buffer)
        win32api.mouse_event(win32con.MOUSEEVENTF_LEFTUP,0,0)
def rightclick(buffer = .1,amount = 1):
    for i in range(amount):
...
def scroll(dir,amt):
    for i in range(amt):
        ...
        win32api.mouse_event(MOUSEEVENTF_WHEEL, x, y, dir, 0)




def press(*args,timeu = .05):
    for i in args:
...
def typer(string=None, timeu = .05, *args):
    for i in string:
        if i == ' ':
